...
const STORY_CONFIG = COMPONENT_CSS_VAR_MAPPING.storyCard;

test.describe('Story Card Spacing', () => {
  test.describe('.story-card__content padding', () => {
    const config = STORY_CONFIG.content;
    ...
    for (const [bpName, bp] of Object.entries(BREAKPOINTS)) {
      test(`should use ${config.padding} at ${bpName} (${bp.width}px)`, async ({ page }) => {
        ...
        await page.goto(`${BASE_URL}${TEST_URLS.stories}`);
        await page.waitForLoadState('networkidle');

        // Check if element exists
        const exists = await elementExists(page, config.selector);
        skipIfNotFound(exists, 'Story card content');
...
test.describe('Story Card - Variable Binding Validation', () => {
  test('content padding should match --padding-e-inner at 1024px', async ({ page }) => {
    await page.setViewportSize({ width: 1024, height: 768 });
    await page.goto(`${BASE_URL}${TEST_URLS.stories}`);
    await page.waitForLoadState('networkidle');

    const exists = await elementExists(page, STORY_CONFIG.content.selector);
    skipIfNotFound(exists, 'Story card content');
    ...
    console.log('Story card content at 1024px:');
  ...
  test('content padding should match --padding-e-inner at 1440px', async ({ page }) => {
    ...
    console.log('Story card content at 1440px:');
    console.log(`  Computed padding: ${result.computedValue}px`);
    console.log(`  Expected (--padding-e-inner): ${result.expectedValue}px`);
...
test.describe('Story Card - All Breakpoints Summary', () => {
  test('generate spacing summary for story card', async ({ page }) => {
    const summary: Array<{
      breakpoint: string;
      width: number;
    ...
    for (const [bpName, bp] of Object.entries(BREAKPOINTS)) {
      ...
      if (exists) {
        result = await verifyCSSVariableUsage(
          ...
          STORY_CONFIG.content.selector,
          'padding-top',
          STORY_CONFIG.content.padding as any,
          bp.width,
          TOLERANCE
        );
      }
